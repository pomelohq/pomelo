...
struct WorkTab: Identifiable, Hashable {
    enum Kind: Hashable { case file(repo: String, path: String), search, gitDiff(repo: String, path: String), terminal(holder: String) }
    var kind: Kind
    var label: String
    ...
    var id: String {
        ...
        case .terminal(let holder): return "term:\(holder)"
        }
    }
...
@Observable final class Workbench {
    ...
    var openTerminalCmd = 0                   // "+ New Terminal" -> open a terminal tab in this pane
