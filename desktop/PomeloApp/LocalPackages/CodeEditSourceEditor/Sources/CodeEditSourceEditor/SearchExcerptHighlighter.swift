...
public enum SearchExcerptHighlighter {
    // Tree-sitter capture spans for a snippet (used to give read-only code surfaces — diffs, etc. — the same
    // tokenization as the editor). Resolves predicates + precedence like the editor's highlighter.
    public static func captureSpans(text: String, path: String) -> [(range: NSRange, capture: CaptureName)]? {
        let lang = CodeLanguage.detectLanguageFrom(url: URL(fileURLWithPath: path))
        guard let tsLanguage = lang.language, let query = TreeSitterModel.shared.query(for: lang.id) else { return nil }
        let parser = Parser()
        ...
        return query.execute(in: tree)
            ...
            .flatMap { $0.captures }
            .reversed()
            .compactMap { cap -> (range: NSRange, capture: CaptureName)? in
                if let level = ranges[cap.range], level <= cap.index { return nil }
                guard let name = CaptureName.fromString(cap.name) else { return nil }
                ranges[cap.range] = cap.index
                return (cap.range, name)
            }
    }
