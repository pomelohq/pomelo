...
use pomelo_editor::{EditorBuffer, EditorRenderer};
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        renderer.set_theme(pomelo_editor::Theme::load_or_default(&theme_config_path()));
        renderer.set_language(pomelo_editor::Lang::from_ext(&self.ext));
...
// User theme override: ~/.config/pomelo/theme.json (falls back to built-in One Dark).
fn theme_config_path() -> std::path::PathBuf {
    let home = std::env::var("HOME").unwrap_or_default();
    std::path::Path::new(&home).join(".config/pomelo/theme.json")
}
