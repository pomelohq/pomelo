...
fn gc(c: crate::theme::Color) -> Color {
        ...
        self.text_renderer.prepare(
            ...
            [
                TextArea {
                    ...
                    scale: self.scale,
                    bounds,
                    default_color: gc(self.theme.gutter),
                    custom_glyphs: &[],
                },
                TextArea {
                    buffer: &self.buffer,
        ...
        let mut encoder = self.device.create_command_encoder(&Default::default());
        {
            let mut pass = encoder.begin_render_pass(&RenderPassDescriptor {
                ...
                color_attachments: &[Some(RenderPassColorAttachment {
                    ...
                    resolve_target: None,
                    ops: Operations {
                        // Editor background from the theme. The surface is sRGB; wgpu gamma-encodes the clear, so pass
                        // a linearized color.
                        load: LoadOp::Clear(wgpu::Color {
                            r: srgb_to_linear(self.theme.background.0[0] as f64 / 255.0),
                            g: srgb_to_linear(self.theme.background.0[1] as f64 / 255.0),
                            b: srgb_to_linear(self.theme.background.0[2] as f64 / 255.0),
                            a: 1.0,
                        }),
                        store: StoreOp::Store,
                    },
