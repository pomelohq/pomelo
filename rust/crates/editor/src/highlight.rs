//! Syntax highlighting via tree-sitter + each grammar's `highlights.scm` query — the same approach as Zed. Each span
...
impl Highlighter {
    ...
    pub fn highlight(&mut self, text: &str, lang: Lang) -> Vec<Span> {
        ...
        let Some(Some(config)) = configs.get(&lang) else {
            return vec![Span { text: text.to_string(), capture: "" }];
        };
        ...
        let events = match inner.highlight(config, text.as_bytes(), None, None, |_| None) {
            Ok(e) => e,
            Err(_) => return vec![Span { text: text.to_string(), capture: "" }],
        };
        for event in events.flatten() {
            match event {
                ...
                HighlightEvent::Source { start, end } => {
                    ...
                    spans.push(Span { text: text[start..end].to_string(), capture });
                }
            }
        }
        spans
