...
func (s *Server) dialOrSpawnHolder(name string, cols, rows int, cwd string) (net.Conn, error) {
	...
	// pre-created holders (services + shortcut/editor/repo shells) are started elsewhere —
	// attach just waits for their socket, never spawning a bare shell over the running command
	if !services.HolderFor(name).AutoSpawnOnAttach() {
